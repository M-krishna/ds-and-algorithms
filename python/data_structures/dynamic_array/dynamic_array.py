...
class DynamicArray:
    ...
    def remove(self, item: int) -> None:
        ...
        if self.count == 0:
            raise Exception(
                IndexError.__doc__
            )  # raise an exception if the array is empty
        ...
        if self.arr[0] == item:
            ...
            for index, value in enumerate(
                self.arr
            ):  # iterating an enumerate object returns both index and value
                if value == item:
                    found = True
                    items_to_the_left = self.arr[0:index]
                    items_to_the_right = self.arr[index + 1 :]
                    self.arr = (
                        items_to_the_left + items_to_the_right
                    )  # merges both array
                    self.pointer -= 1
                    self.count -= 1
            if not found:
...
if __name__ == "__main__":
    ...
    da.remove(
        6
    )  # since 6 is the last element in the array internally it uses pop method to remove it.
    print(da)
    da.remove(3)
    print(da)
    da.remove(4)
